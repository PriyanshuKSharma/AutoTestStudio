...
class SenderPanel(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(
            ...
            text="CAN ID (hex):",
            font=ctk.CTkFont(family="Segoe UI", size=13),
        ).grid(row=2, column=0, padx=(0, 12), pady=8, sticky="w")
        ...
        # DLC Row
        ctk.CTkLabel(
            raw_inner, text="DLC:", font=ctk.CTkFont(family="Segoe UI", size=13)
        ).grid(row=3, column=0, padx=(0, 12), pady=8, sticky="w")
        ...
        ctk.CTkLabel(
            ...
            text="Data (hex bytes):",
            font=ctk.CTkFont(family="Segoe UI", size=13),
        ).grid(row=4, column=0, padx=(0, 12), pady=8, sticky="w")
        ...
        ctk.CTkLabel(
            raw_inner,
            text="Interval (ms, 0=once):",
            font=ctk.CTkFont(family="Segoe UI", size=13),
        ).grid(row=5, column=0, padx=(0, 12), pady=8, sticky="w")
    ...
    def _build_message(self) -> can.Message:
        ...
        data = (
            bytes(int(b, 16) for b in data_str.split())
            if data_str
            else b"\x00" * int(self._dlc_var.get())
        )
        return can.Message(
            arbitration_id=can_id, data=data, is_extended_id=self._ext_var.get()
        )
    ...
    def _send_once(self):
        try:
            ...
            self._status.configure(
                text="[Sent] Frame successfully transmitted.", text_color="green"
            )
    ...
    def _toggle_cyclic(self):
        if self._cyclic_running:
            ...
            self._cyclic_btn.configure(
                text="Start Cyclic", fg_color=None, hover_color=None
            )
            self.snd_dot.configure(fg_color="gray")
            self.snd_text.configure(
                text="Cyclic: Inactive", text_color=("gray50", "gray40")
            )
            self._status.configure(
                text="Stopped cyclic transmission.", text_color=("gray50", "gray40")
            )
        else:
            try:
                ...
                if interval <= 0:
                    self._status.configure(
                        text="[Warning] Interval must be greater than 0 ms.",
                        text_color="orange",
                    )
                    return
                self._cyclic_running = True
                self._cyclic_btn.configure(
                    text="Stop Cyclic", fg_color="#dc2626", hover_color="#ef4444"
                )
                self.snd_dot.configure(fg_color="#10b981")
                self.snd_text.configure(text="Cyclic: Active", text_color="#10b981")
                self._status.configure(
                    text=f"Sending cyclic frames every {interval} ms...",
                    text_color="green",
                )
    ...
    def _encode_fill(self):
        try:
            ...
            self._status.configure(
                text="[Encoded] DBC encoding successful. Form filled.",
                text_color="green",
            )
        except Exception as e:
