...
class TestRunnerPanel(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        self._results_frame = ctk.CTkScrollableFrame(
            results_inner, height=130, fg_color="transparent"
        )
    ...
    def _on_done(self, rc: int):
        ...
        if rc == 0:
            self._status_label.configure(
                text="[PASSED] Script finished successfully.", text_color="#10b981"
            )
        else:
            self._status_label.configure(
                text=f"[FAILED] Script failed (exit code {rc}).", text_color="#ef4444"
            )
        self._load_results()
    ...
    def _load_results(self):
        ...
        db = get_db()
        rows = db.execute(
            "SELECT timestamp, test_name, status FROM test_results ORDER BY id DESC LIMIT 20"
        ).fetchall()
        for idx, r in enumerate(rows):
            row_bg = ("gray95", "gray24") if idx % 2 == 0 else "transparent"
            row_frame = ctk.CTkFrame(
                self._results_frame, fg_color=row_bg, corner_radius=4
            )
