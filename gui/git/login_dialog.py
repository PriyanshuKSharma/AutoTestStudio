...
class LoginDialog(ctk.CTkToplevel):
    ...
    def _build(self) -> None:
        ctk.CTkLabel(
            self, text="GitHub Authentication", font=ctk.CTkFont(size=16, weight="bold")
        ).pack(pady=(24, 4))
        ctk.CTkLabel(
            self,
            text="Enter your GitHub username and Personal Access Token.",
            text_color="gray",
        ).pack(pady=(0, 16))
        ...
        self._username = ctk.CTkEntry(
            form, width=240, placeholder_text="github-username"
        )
        ...
        self._pat = ctk.CTkEntry(
            form, width=240, show="*", placeholder_text="ghp_xxxxxxxxxxxx"
        )
        ...
        ctk.CTkCheckBox(
            self,
            text="Remember login (stored securely in OS keyring)",
        ...
        ctk.CTkButton(btn_row, text="Login", width=100, command=self._login).pack(
            side="left", padx=8
        )
    ...
    def _login(self) -> None:
        ...
        if not git_service.validate_credentials(username, pat):
            self._status.configure(
                text="Authentication failed. Check username and token.",
                text_color="red",
            )
            return
