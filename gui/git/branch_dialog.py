...
class BranchDialog(ctk.CTkToplevel):
    ...
    def _build(self) -> None:
        ctk.CTkLabel(
            self, text="Branch Management", font=ctk.CTkFont(size=16, weight="bold")
        ).pack(pady=(20, 4))
        ctk.CTkLabel(
            self, text="Never push directly to main or master.", text_color="gray"
        ).pack(pady=(0, 12))
        ...
        try:
            ...
            current = "unknown"
        ctk.CTkLabel(
            self, text=f"Current Branch:  {current}", font=ctk.CTkFont(weight="bold")
        ).pack(pady=4)
        ...
        ctk.CTkLabel(
            self, text="Create New Branch", anchor="w", font=ctk.CTkFont(weight="bold")
        ).pack(anchor="w", padx=24)

        new_row = ctk.CTkFrame(self, fg_color="transparent")
        new_row.pack(fill="x", padx=24, pady=6)
        self._new_branch = ctk.CTkEntry(
            new_row, width=280, placeholder_text="feature/my-test-branch"
        )
        self._new_branch.pack(side="left", padx=(0, 8))
        ctk.CTkButton(
            new_row, text="Create", width=80, command=self._create_branch
        ).pack(side="left")
        ...
        ctk.CTkButton(
            btn_row, text="Select Branch", width=120, command=self._confirm
        ).pack(side="left", padx=8)
    ...
    def _confirm(self) -> None:
        ...
        if not branch or branch in _PROTECTED:
            self._status.configure(
                text="Select a valid feature branch (not main/master).",
                text_color="red",
            )
            return
