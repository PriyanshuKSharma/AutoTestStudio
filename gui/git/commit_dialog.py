...
class CommitDialog(ctk.CTkToplevel):
    ...
    def _build(self) -> None:
        ctk.CTkLabel(
            self, text="Commit Details", font=ctk.CTkFont(size=16, weight="bold")
        ).pack(pady=(20, 4))
        ctk.CTkLabel(self, text=f"Branch:  {self._branch}", text_color="gray").pack(
            pady=(0, 12)
        )
        ...
        ctk.CTkButton(
            self,
            text="Restore Suggestion",
            fg_color="gray40",
            width=160,
            command=self._restore_suggestion,
        ).pack(pady=4)
