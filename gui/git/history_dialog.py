...
class HistoryDialog(ctk.CTkToplevel):
    ...
    def _build(self) -> None:
        ...
        ctk.CTkLabel(
            top, text="Version History", font=ctk.CTkFont(size=16, weight="bold")
        ).pack(side="left")
        ctk.CTkButton(top, text="Refresh", width=80, command=self._load).pack(
            side="right"
        )
        ...
        for col, w in [
            ...
            ctk.CTkLabel(
                header, text=col, width=w, anchor="w", font=ctk.CTkFont(weight="bold")
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            ctk.CTkLabel(
                self._scroll, text=f"Could not load history: {exc}", text_color="red"
            ).pack(pady=20)
            return

        if not records:
            ctk.CTkLabel(
                self._scroll, text="No commits found.", text_color="gray"
            ).pack(pady=20)
            return
